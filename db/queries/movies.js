...
export async function createMovie (title, genre, runtime, releaseDate, director, moviePoster, plotSummary) {
  const sql = `
    INSERT INTO movies (title, genre, runtime, release_date, director, movie_poster, plot_summary)
    VALUES ($1, $2, $3, $4, $5, $6, $7)
    RETURNING *
  `;

  const { rows: [newMovie] } = await db.query(sql, [title, genre, runtime, releaseDate, director, moviePoster, plotSummary]);
...
export async function updateMovieById (title, genre, runtime, releaseDate, director, moviePoster, plotSummary) {
const sql = `UPDATE movies SET title=$2, genre=$3, runtime=$4, release_date=$5, director=$6, movie_poster=$7, plot_summary=$8
WHERE id=$1
RETURNING *`;

const { rows: [updatedMovie] } = db.query(sql, [title, genre, runtime, releaseDate, director, moviePoster, plotSummary]);
